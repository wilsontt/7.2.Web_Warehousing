...
**狀態**: 草稿 (Draft)  
**建立日期**: 2025-11-28  
**最近修訂**: 2025-11-28 (引用通用欄位映射規範)
**依據**:
- `docs/0.standards/0.專案憲章-總規範-SDD工作流程/1.專案憲章 Project Constitution.md` (v2.2.0)
- `docs/0.standards/0.專案憲章-總規範-SDD工作流程/2.文件倉儲管理系統 Warehouse Management System 設計規範v2.0.md`
- `docs/0.standards/4.資料庫結構分析/1.代碼維護/CDF_代碼檔1_*.md`
- `docs/0.standards/4.資料庫結構分析/1.代碼維護/CDS_代碼檔2_*.md`
- `docs/0.standards/4.資料庫結構分析/1.代碼維護/CDT_代碼檔3_*.md`
- `docs/0.standards/3.8大核心功能/1.基本作業/代碼維護/1.1.代碼維護.png` (舊系統參考畫面)
...
erDiagram
    ...
    CDF {
        string CDF00 PK "大分類編碼"
    }
    CDS {
        string CDF00 FK
        string CDS00 PK "中分類編碼"
    }
    CDT {
        string CDF00 FK
        string CDS00 FK
        string CDT00 PK "細分類編碼"
    } 
```
...
> **[遵循通用規範]** 
> 本章節遵循 `0.standards/0.專案憲章-總規範-SDD工作流程/3.規格驅動開發 (SDD) 工作流程.md` 第 3.2 節定義的「前後端欄位映射規範」。
> 
> **[重要安全設計]** 依據 **P2 設計即安全** 原則：
> - 前端**禁止**直接使用資料庫欄位名稱（如 `CDF00`）。
> - 所有欄位必須使用「英文欄名 (Lovable)」進行映射（如 `CategoryFirstNo`）。
> - 前端元件、預設值與唯讀狀態**必須**遵循下表定義。
...
**驗收條件**:
1.  **[UI 佈局]** 採用三欄式佈局（左：大分類 / 右上：中分類 / 右下：細分類），符合 `docs/0.standards/3.8大核心功能/1.基本作業/代碼維護/1.1.代碼維護.png` 參考畫面。
2.  **[聯動行為]**
...
### 5.1 佈局結構
參考 `docs/0.standards/3.8大核心功能/1.基本作業/代碼維護/1.1.代碼維護.png`，B 區分為三塊：
