...
**狀態**: 草稿 (Draft)
**建立日期**: 2025-11-27
**依據**: `docs/1.specs/2.登入首頁.spec.md`
...
- **變更**:
    ...
    - **UI 優化**:
        - 引入 `lucide-react` 圖示庫，為 8 大模組配置對應 Icon。
        - 模組名稱前加入數字編號 (1. ~ 8.) 以提升辨識度。
    - 實作遞迴或多層級 Dropdown UI 以支援巢狀選單 (如：基本作業 -> 代碼作業 -> 基本代碼維護)。
    - **使用者選單整合**:
        - 將「語系切換」與「深淺色切換」功能移入使用者下拉選單。
        - **新增「字體大小」調整功能**:
            - 選項：小 (14px)、中 (16px)、大 (18px)。
            - 實作：透過修改 `:root` 的 `--base-font-size` CSS 變數，連動 `html` 根元素字體大小，利用 Tailwind `rem` 機制達成全站縮放。
            - 持久化：設定值需儲存於 `localStorage`。
        - 調整選單順序：使用者資訊 -> 語系/主題/字體 -> 變更密碼 -> 登出。
    - 優化「使用者選單」下拉樣式。
